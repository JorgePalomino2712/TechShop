...
export default function CardsP() {
    ...
    useEffect(() => {
        async function traerCards() {
            try {
                ...
                console.log(data);
                const filtrados = data.filter(item => item.category === "audio");
    ...
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 p-5 items-center justify-center'>
            {error && <p className="text-red-400 text-sm text-center mt-2">{error}</p>}
            {product.map((p) => (
                <div key={p.id} className="w-60 h-80 bg-gray-50 p-3 flex flex-col gap-1 rounded-2xl ">
